...
| Reachability | **Fixed 2026-07-26 — the install-reachability breaks are closed; F4 still has no shipped consumer (see below).** (1) The relay resolved `qdistro_admin_rules` via a checkout-only `../broker` path while being installed to `/usr/local/lib/qdistro`, so the import always failed, the failure was swallowed, and the gate denied unconditionally; the relay now installs to `/usr/libexec/qdistro` beside the broker modules and an import failure is loud (distinct reason `rules-import-error`, an ERROR line at module import, another in the daemon's startup preflight, and an ERROR on the first denied call followed by a rate-limited reminder carrying the suppressed count). (2) `install-user-relay-for-vm.sh` was in no production chain; it is now in `qdistro-bootstrap.sh`'s `installer_chain_entries` (both profiles) and in `image/config.sh`. (3) **F4-a is fixed:** `org.qdistro.UserRelay.conf` used to grant `<allow own=...>` to exactly two identities — username `work` for `uid2000` and `work2` for `uid3000` — keyed on the USERNAME, and nothing in the install chain ever creates those users, so on any real system every silo's relay was refused its name and exited 78. The grant is now per-silo and issued with the silo: `qdistro-session-manager` writes `/etc/dbus-1/system.d/org.qdistro.UserRelay.silo-<name>.conf` on CreateSilo, removes it on DeleteSilo, and reconciles the whole set against `silos.yaml` at daemon startup (the upgrade path — every pre-existing silo lacks a grant). The static grants were deleted rather than extended: keeping them would go on granting relay-name ownership to whoever happens to be named `work`. (4) **F4-b is fixed:** `image/config.sh` staged the relay but not `install-session-manager.sh`, so on the kiwi release image the relay template was present and nothing created a silo, started a relay, or issued a grant; the session manager is now in the image's `INSTALLERS` list. Coverage: `tests/unit/test_user_relay_installed_layout.py`, `tests/unit/test_relay_dbus_policy.py` (the latter includes a probe that feeds a generated fragment to a real `dbus-daemon`, plus an anti-vacuity check that the probe can fail). **Still true:** the only in-tree caller of `ForwardBrowserBridgeOp` is the Recall admin surface, which is post-v1 (see the *First consumer* row) — F4 is a capability with no shipped consumer, so it is reachable but not yet claimable as a user-visible v1 feature. Source: `todo/fable-release/10-reachability-audit-2026-07-26.md`, F4 row. |
