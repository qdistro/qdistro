...
(function (root) {
  ...
  // CLOSED BY DEFAULT (opus HIGH J11, ported from qdchrome-extension —
  // iso2 `06` F1). An empty / unset allowlist denies every origin — a
  // fresh install must not ship the page-initiated credential/extraction
  // surface open to every site. A single `*` entry is the explicit opt-in
  // to "all origins" (the pre-J11 wide-open behaviour, now a deliberate
  // user choice) and short-circuits to allow, matching the old
  // empty-means-all semantics including opaque URLs. Otherwise the url
  // must match a listed entry. A URL we can't parse (about:, blank tab,
  // opaque origin) is REJECTED unless `*` is set — the allowlist is an
  // explicit "only these origins" instruction.
