# Agent instructions for writing qdfirefox-extension tests

You are adding or changing tests in this repo (the qdistro native-messaging
WebExtension for Firefox, MV3 event page). Read this before you touch anything
under `tests/`. This is a **JS/TS** project run by **vitest**, not pytest — the
qdistro `@pytest.mark.cheat_aware` marker does **not** apply here (see the note
at the end). The same *discipline* still does.
...
- Layout: flat `tests/*.test.js`, one file per surface (e.g.
  ...
  (`loadExtension`, `makeFakePort`, `makeFakeBrowser`); use these instead of
  hand-rolling a fake `browser`/port per file. Note the Firefox fakes return
  **Promises** from API methods (the Firefox-native shape) because
  `src/api.js` binds to `browser` and modules `await` the results directly —
  this is the key difference from the qdchrome-extension helpers, which use
...
(`src/dispatcher.js`), and its manifest declares `nativeMessaging` plus
...
- the declared `permissions` / `host_permissions` and the pinned
  `browser_specific_settings.gecko.id` asserted in `manifest.test.js`.

Treat a loosened op-routing match, a dropped unknown-op rejection, a widened
permission set, a dropped gecko-id pin, or a heartbeat/timeout deadline bumped
to hide a hang the same way qdistro treats a broadened qsu approval: a denial
or boundary that stops being enforced is a coverage regression, not a fix. If
one of these genuinely needs to change, justify it against a product change in
your report.
