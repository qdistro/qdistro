...
The caller supplies a 32-byte session secret through the same sealed launcher
boundary as the pairing snapshot.  TLS (or another encrypted carrier) is still
required for confidentiality; the HMAC envelope supplies end-to-end identity,
channel separation, and replay protection even if a carrier is miswired.
"""
...
class AuthenticatedEnvelopeCodec:
    ...
    def __init__(self, binding: RemoteSessionBinding, master_key: bytes):
        binding.validate()
    ...
    def _key(self, direction: str, channel: str) -> bytes:
        ...
        return hmac.new(self._master_key, b"envelope-key\0" + context,
...
class RemoteProxyLifecycle:
    ...
    def announce(self, *, epoch: int, source_revision: int) -> None:
    ...
    def source_closed(self, *, source_revision: int) -> None:
