...
# ---------------------------------------------------------------------------
#
# Each silo runs qdistro-user-relay@<uid>.service, which must own the
# system-bus name org.qdistro.UserRelay.uid<N>. The system bus denies
# owning any name by default, so that grant has to exist as policy.
#
# Until 2026-07-26 the grant was a static pair in
# user_relay/org.qdistro.UserRelay.conf: username "work" for uid 2000 and
# username "work2" for uid 3000. Nothing creates those users — bootstrap
# makes only admin(1000) and the regular user(1001) — so the grant matched
# a silo only by coincidence of naming. Every silo with any other name, and
# every uid outside {2000, 3000}, was refused, the relay exited 78, and
# both cross-silo Send-To and the Firefox-containers cross-uid opt-in (F4)
# were dead on any real install.
#
# The grant is per-silo state, so it is issued and revoked with the silo,
# by the component that owns the silo lifecycle. Files land directly in
# /etc/dbus-1/system.d/ — the same directory, and therefore the same
# already-proven load path, as the static conf; no <includedir> support is
# assumed of the bus implementation. The RELAY_POLICY_PREFIX namespace is
# what makes the generated files enumerable for the startup reconcile and
# distinguishable from anything shipped.

RELAY_POLICY_DIR = Path("/etc/dbus-1/system.d")
RELAY_POLICY_PREFIX = "org.qdistro.UserRelay.silo-"
RELAY_BUS_NAME_FMT = "org.qdistro.UserRelay.uid{uid}"


def relay_policy_filename(name: str) -> str:
    """Basename of the generated policy fragment for silo `name`."""
    return f"{RELAY_POLICY_PREFIX}{validate_name(name)}.conf"


def relay_policy_silo_name(filename: str) -> str | None:
    """Inverse of relay_policy_filename: the silo a generated fragment
    belongs to, or None if `filename` is not one of ours. Used by the
    startup reconcile to spot fragments whose silo is gone."""
    if not filename.startswith(RELAY_POLICY_PREFIX) or \
            not filename.endswith(".conf"):
        return None
    name = filename[len(RELAY_POLICY_PREFIX):-len(".conf")]
    try:
        return validate_name(name)
    except BadArgument:
        # A file in our namespace whose stem is not a legal silo name was
        # not written by us. Leave it alone rather than delete it.
        return None


def relay_policy_xml(name: str, uid: int) -> str:
    """The busconfig fragment granting silo `name` (uid `uid`) ownership of
    its own relay name, and root the right to talk to it.

    Both arguments are re-validated here even though every caller has
    validated them already: this string is concatenated into XML that
    becomes system-bus policy, so the check that keeps it injection-free
    belongs at the point of use, not only at the point of entry.
    """
    name = validate_name(name)
    uid = validate_uid(uid)
    bus_name = RELAY_BUS_NAME_FMT.format(uid=uid)
    return f"""<!DOCTYPE busconfig PUBLIC
 "-//freedesktop//DTD D-Bus Bus Configuration 1.0//EN"
 "http://www.freedesktop.org/standards/dbus/1.0/busconfig.dtd">
<busconfig>
  <!-- GENERATED by qdistro-session-manager for silo "{name}" (uid {uid}).
       Do not edit: rewritten when the silo is created, removed when it is
       deleted, and reconciled against silos.yaml at daemon startup. -->

  <!-- The broker (root) calls ListLocalReceivers + Forward on the relay. -->
  <policy user="root">
...
class _SystemOps:
    ...
    def useradd(self, name: str, uid: int) -> None:
        ...
        # not yet grant this silo anything, and create() then never reloaded
        # again, so the grant only took effect at the next unrelated reload.
    ...
    def write_relay_policy(self, name: str, uid: int,
        ...
        # Measured (dbus-daemon 1.14): a malformed file under an <includedir>
        # is NOT fatal — the bus logs "Encountered error ... while parsing"
        # and starts anyway. That is worse for us, not better: the silo ends
        # up with no grant, its relay exits 78, and the only trace is one
    ...
    def list_relay_policies(self) -> list[str]:
        ...
        try:
            entries = os.listdir(RELAY_POLICY_DIR)
        except OSError:
            return []
        out = []
        for fn in entries:
            silo = relay_policy_silo_name(fn)
            if silo is not None:
                out.append(silo)
        return out

    def relay_policy_uids(self, name: str) -> list[int] | None:
        """EVERY numeric `<policy user="N">` an existing fragment carries, or
        ...
        backup. Such a file is not merely wrong: it aborts dbus-broker at its
        next reload, so "unreadable" and "unparseable" both come back as None
        and are treated as unsafe rather than ignored.

        All of them, not the first. A file we generated has exactly one
        numeric user policy, but this function exists precisely for files
        that are no longer what we generated. A hand edit that leaves a
        resolvable numeric policy followed by an unresolvable one would look
        safe if we stopped at the first match, and the second one is just as
        fatal to the bus as the first.
        """
        p = RELAY_POLICY_DIR / relay_policy_filename(name)
        try:
...
class _SiloStore:
        ...
        try:
            with self._lock:
                ...
                try:
                    ...
                    # Revoke the relay bus-name grant BEFORE userdel, so the
                    ...
                    log.error("delete of silo %r failed during teardown: %s",
                              silo.name, e)
                    # The silo survives (rolled back to Stopped), so its relay
                    # grant has to survive with it. Without this re-issue a
                    # failed delete would leave a live silo whose relay can
                    # never claim its name again until the daemon restarts.
                    if silo.kind == KIND_TIER3_USER:
                        try:
                            self._ops.write_relay_policy(silo.name, silo.uid)
                        except Exception as reissue:  # noqa: BLE001
                            log.error(
                                "could not re-issue the user-relay policy for "
    ...
    def reconcile_relay_policies(self) -> tuple[list[str], list[str]]:
        ...
        Issues a fragment for every tier-3 silo that lacks one and removes
        every fragment in our namespace whose silo no longer exists.

        This is the upgrade path, and it is the reason the fix is not just
        "write the file in create()": every silo that already exists on an
        installed system was created before this code, so none of them has a
        grant. Without a reconcile the fix would only work for silos created
        after the upgrade — which is the same shape of defect (correct code
        that never runs for the population that needs it) this whole
    ...
    def _purge_unsafe_relay_fragments(self) -> tuple[list[str], list[str]]:
        ...
        reloads a fragment whose numeric `user=` NSS cannot resolve (see
        write_relay_policy). Combined with dbus-broker-launch's inotify watch
        on /etc/dbus-1/system.d, that makes the whole directory the hazard
        surface: any write in there, for any silo, can trigger the reload
        that detonates a bad file left by something else entirely — an
        out-of-band `userdel`, a restore from backup, a torn write.

        So every path that touches this directory purges first. `unpurged`
        ...
        try:
            ...
            log.warning("relay-policy purge: listing failed: %s", e)
            return ([], [])
        revoked: list[str] = []
        unpurged: list[str] = []
        for name in have:
            ...
            if reason is None:
                # Row and account agree. The FILE may still name a uid that
                # does not resolve — a hand edit, a torn write — and that is
                # the shape that aborts the bus.
                try:
                    uids = self._ops.relay_policy_uids(name)
                    if uids is None:
                        reason = "fragment unreadable or unparseable"
                    elif not uids:
                        reason = "fragment grants no numeric user policy"
                    else:
                        bad = [u for u in uids if not self._ops.uid_exists(u)]
                        if bad:
                            reason = (f"fragment names uid(s) "
                                      f"{', '.join(str(u) for u in bad)} "
                                      f"with no passwd entry")
                except Exception as e:  # noqa: BLE001
                    reason = f"could not read the fragment ({e})"
            if reason is None:
                continue
            log.warning("relay-policy purge: removing the fragment for %r "
                        "(%s)", name, reason)
            try:
        ...
        return (revoked, unpurged)

    def _unsafe_fragment_error(self, unpurged: list[str]) -> str:
        return (
            "the D-Bus policy directory holds a relay fragment that could "
            "not be removed (" + ", ".join(repr(n) for n in unpurged) + "). "
            "It may name a uid with no passwd entry, and writing or "
            "reloading policy while it is present can abort the system bus. "
    ...
    def _reconcile_relay_policies_locked(self) -> tuple[list[str], list[str]]:
        ...
        writes the fragments that should exist. Nothing is written until the
        purge has finished, because any write can trigger the reload that
        aborts the bus on a bad file already present — see
        _purge_unsafe_relay_fragments. If the purge could not remove
        something, this writes nothing and reloads nothing: every such action
        is a chance to detonate the one we could not remove.

        This is also the upgrade path. Every silo on an already-installed
        system was created before this mechanism existed, so none of them has
        a grant; without a reconcile the fix would only ever work for silos
        created after the upgrade, which is the same defect class one turn
        later.
        """
        revoked, unpurged = self._purge_unsafe_relay_fragments()
        issued: list[str] = []
        if unpurged:
            log.error("relay-policy reconcile: aborting — %s",
        ...
        try:
            ...
            log.warning("relay-policy reconcile: listing failed: %s", e)
            return (issued, revoked)

        for name, uid in sorted(want.items()):
            if name in have:
                # Survived the purge, so its identity and its uids are both
                # good. Only the exact expected CONTENT is left to check —
                # presence alone would let a drifted fragment through, since
                # the filename carries the name while the content carries the
                # uid the granted bus name is built from.
                try:
                ...
                # No fragment: it never had one (the upgrade path), or the
                # purge just removed a bad one. Either way the identity has
                # to be established before we grant.
                try:
