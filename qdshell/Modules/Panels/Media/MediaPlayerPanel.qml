...
SmartPanel {
  ...
  panelContent: PanelShell {
    ...
    title: I18n.tr("common.media-player")
    icon: "music"
    onCloseRequested: root.close()

    // Override PanelShell's default size calc with Media's mode-driven height.
    contentPreferredHeight: (root.compactMode ? 240 : (root.showAlbumArt ? 560 : 300)) * Style.uiScaleRatio
    ...
    // Player selector chip — visible only when multiple media players are active.
    headerActions: [
      Rectangle {
