...
SmartPanel {
  ...
  panelContent: PanelShell {
    ...
    title: I18n.tr("panels.audio.title")
    icon: "settings-audio"
    ...
    // Tabs (Volumes / Devices). Originally lived inside the header NBox; lifted
    // out into its own card now that PanelShell owns the title row.
    NBox {
      Layout.fillWidth: true
      Layout.preferredHeight: tabBar.implicitHeight + Style.marginS * 2
