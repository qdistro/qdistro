...
// This file is a hand-kept copy of the pure logic in Commons/FuzzySort.qml.
// Commons/FuzzySort.qml does NOT import this module; it contains the logic
// directly. Tests (tests/test_fuzzysort.js) require this file under Node.
//
// DRIFT RISK: if the logic in Commons/FuzzySort.qml changes, this file must
// be updated manually. tests/test_drift_guard.js asserts that the function
// bodies match the QML source to catch silent drift.
//
// Known intentional deviations from the QML source (see each for rationale):
//   1. go() calls _resetQueue() before the main loop. The QML singleton owns
//      a shared _q property that is reset between requests implicitly because
//      QML re-initialises property var per component instance. Under Node the
//      module is loaded once and go() can be called many times in one process,
//      so _resetQueue() is needed to avoid cross-call contamination.
//   2. _isPrepared() adds `x !== null` guard before checking `x._bitflags`.
//      The QML version omits it because QML's property system protects against
//      null object access differently; in plain JS `null._bitflags` throws.
//
// Extraction rule: every `function _foo` and `property var _bar` that
// carried pure logic was copied verbatim; QML-only property declarations
// (`property var _matchesSimple: []`) became plain `var` initialisations.
// The `root` forward-reference in `_createResult` / `_createKeysResult`
// is replaced by a direct reference to the local functions which is the
// only structural change required by the Node module system.

"use strict";

var _INFINITY = Infinity;
var _NEGATIVE_INFINITY = -Infinity;
var _NULL = null;
...
var _matchesSimple = [];
var _matchesStrict = [];
var _nextBeginningIndexesChanges = [];
var _keysSpacesBestScores = [];
var _allowPartialMatchScores = [];
var _tmpTargets = [];
var _tmpResults = [];
...
var _q = _fastpriorityqueue();

// Reset the queue (needed between go() calls in test environments where the
// module is required once but go() is called many times).
function _resetQueue() {
  _q = _fastpriorityqueue();
}
...
function _createResult() {
  return {
    ...
    _bitflags: 0,
    get indexes() {
      return this._indexes.slice(0, this._indexes.len).sort(function(a, b) { return a - b; });
    },
    set indexes(idx) {
      this._indexes = idx;
    },
    highlight: function (open, close) {
      return _highlight(this, open, close);
    },
    get score() {
      return _normalizeScore(this._score);
    },
    set score(s) {
      this._score = _denormalizeScore(s);
    }
  };
}
...
function _new_result(target, options) {
  ...
  result.obj = (options.obj !== undefined ? options.obj : _NULL);
  result._score = (options._score !== undefined ? options._score : _NEGATIVE_INFINITY);
  ...
  result._targetLowerCodes = (options._targetLowerCodes !== undefined ? options._targetLowerCodes : _NULL);
  result._nextBeginningIndexes = (options._nextBeginningIndexes !== undefined ? options._nextBeginningIndexes : _NULL);
...
var _preparedSearchCache = new Map();
var _noTarget = _prepare('');
...
function _isPrepared(x) {
  return typeof x === 'object' && x !== null && typeof x._bitflags === 'number';
}
...
function _all(targets, options) {
  ...
  var limit = (options && options.limit) ? options.limit : _INFINITY;
  ...
  if (options && options.key) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      for (var keyI = options.keys.length - 1; keyI >= 0; --keyI) {
        var t = _getValue(obj, options.keys[keyI]);
        if (!t) {
...
function go(search, targets, options) {
  _resetQueue();
  if (!search)
    return (options && options.all) ? _all(targets, options) : (function() { var r = []; r.total = 0; return r; })();
  ...
  if (options && options.key) {
    ...
    outer: for (var i = 0; i < targetsLen; ++i) {
      ...
      for (var keyI = 0; keyI < keysLen; ++keyI) {
        var tgt = _tmpTargets[keyI];
        if (tgt === _noTarget) {
          _tmpResults[keyI] = _noTarget;
          continue;
        }
        _tmpResults[keyI] = _algorithm(preparedSearch, tgt, false, containsSpace);
  ...
  if (resultsLen === 0)
    return (function() { var r = []; r.total = 0; return r; })();
