...
# §A): under QCI_RELEASE=1 a `blocked` row in a release-relevant gate is FATAL,
# not exit-0, so "green" for the RC battery means ZERO blocked rows. NO VM. The
...
@test "release-profile: QCI_RELEASE=1 — a blocked release row is fatal (exit 15)" {
    ...
    row_is "blocked-rows" fail          # ...and the escalation row is recorded
    row_note_has "blocked-rows" "release-manifest/unpopulated"
}
...
@test "release-profile: QCI_RELEASE=1 escalation names the fatal gate scope" {
    ...
    row_note_has "blocked-rows" "QCI_RELEASE=1"
}
...
# fully-signed + complete manifest fixture (no blocked rows). It is not retested
