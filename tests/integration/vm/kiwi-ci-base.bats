...
@test "bootstrap: ensures CI extras after masking greetd, before fetching tarballs" {
    ...
    local extras_line fetch_line mask_line
...
@test "live clone from imported kiwi base answers qemu-guest-agent" {
    ...
    local img="${QDWIN_IMG_DIR:-$HOME/.local/share/libvirt/images}/qdistro-kiwi-base.qcow2"
    if [ ! -f "$img" ]; then
    ...
    # qga is up (clone-baseweed already waited). Ask the guest who it is.
    ...
    rm -f "${QDWIN_IMG_DIR:-$HOME/.local/share/libvirt/images}/${vm}.qcow2" \
          "${QDWIN_IMG_DIR:-$HOME/.local/share/libvirt/images}/${vm}.nvram.fd"
    [ "$rc" -eq 0 ]
