...
| `QDISTRO_VM_BASE` | auto | `auto`: clone qci workers from the imported tester image (`qdistro-kiwi-base.qcow2`) if present, else `baseweed-baked`. `kiwi` requires the import (`scripts/vm/import-kiwi-base.sh`). `baked` always uses baseweed-baked. `build-in-vm.sh` always clones baked. |
