...
| `build.sh` | in-VM kiwi driver (also the host-side sync). `--sync-only` copies git's view of the monorepo (tracked + untracked-not-ignored files; never build dirs, caches, `.worktrees/`, `ci/runs/`, keys) into a fresh `root/root/qdistro-src/` and writes the source manifest; `--snapshot-id` prints the pin; the build runs `kiwi-ng system build` then `kiwi-ng result bundle --id <snapshot>` (xz `--threads=0` of the raw + `.sha256`) into `$BUILD_DIR/bundle/`. |
