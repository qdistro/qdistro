...
#
# What it does:
#   - Pins the one source repository the bootstrap fetches: the qdistro
#     monorepo (every component — qdwin, qdshell, qdlocker, qdbrowser,
#     qdgreeter, qdterm, qnotebook, qdfileman and the browser extensions —
#     is in-tree and covered by that one commit).
#   - Resolves the current HEAD commit SHA of the monorepo checkout at
#     --repo-root (default: the repo containing this script, matching the
#     bootstrap's REPO_ROOT default), overridable.
...
# non-blank line must be `<repo> <40-hex-sha>` for a known repo (i.e. `qdistro`;
# a pre-monorepo per-component line is rejected) (no dup repos),
...
# scripts/install/<this> -> the qdistro monorepo root, matching
# qdistro-bootstrap.sh's REPO_ROOT default.
...
# Authoritative repo set — what qdistro-bootstrap.sh's fetch_sources pins.
# Since the monorepo migration that is ONE repository: qdistro. The browser
# extensions (pinned before because v1 users MANUALLY load an extension built
# from that source, doc/browser-extension-install.md) and every other
# component are in-tree, so the qdistro pin already makes them traceable to
# the signed release.
QDISTRO_REPOS=(
    qdistro
)
...
generate_manifest() {
    ...
    for repo in "${QDISTRO_REPOS[@]}"; do
        # The monorepo root itself (was: one sibling checkout per repo).
        dir="$REPO_ROOT"
...
# from the qdistro monorepo checkout at: $REPO_ROOT
