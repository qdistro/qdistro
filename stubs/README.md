...
real apps under the full `com.qdistro.App1` contract.
...
- `qstub_notepad.py` — one window, one text area; claims
 `com.qdistro.StubNotepad.uid<N>` on the session bus and appends
...
The `com.qdistro.UserRelay` session-bus daemon (one per uid) is
